...
public class UpdateActivity extends ListActivity {
		...
		try {
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				String name = doc.getElementsByTagName("name")
						.item(i).getFirstChild().getNodeValue();
				// 图片路径
				String pic = doc.getElementsByTagName("pic")
						.item(i).getFirstChild().getNodeValue();
				// 价格
				...
				int typeId = Integer.parseInt(doc.getElementsByTagName("typeId")
						.item(i).getFirstChild().getNodeValue());
				// 备注
				String remark = doc.getElementsByTagName("remark")
						.item(i).getFirstChild().getNodeValue();				
				// 添加到ContenValues对象
				values.put("_id", id);
	...
	private void updateTable() {		
		...
		try {
			...
			Log.i("UPDATE_TABLE", "nl.getLength="+Integer.toString(nl.getLength()));
			// 循环将数据保存到餐桌表
			for (int i = 0; i < nl.getLength(); i++) {
				// 实例化ContentValues
				ContentValues values = new ContentValues();
				// 解析XML文件获得餐桌id
				int id = Integer.parseInt(doc.getElementsByTagName("tid")
						.item(i).getFirstChild().getNodeValue());
				// 编号
				...
				Log.i("UPDATE_TABLE", "ID="+Integer.toString(id)+" NUM"+Integer.toString(num));
				// 添加到ContenValues对象
				values.put("_id", id);
				values.put("num", num);
